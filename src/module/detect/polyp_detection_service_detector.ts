...
export class PolypDetectionServiceDetectorImpl
    implements PolypDetectionServiceDetector
{
    private readonly imageTypeIdToServiceDM: Map<
        number,
    ...
    constructor(
        ...
        for (const imageTypeId of applicationConfig.imageTypeIdListForPolypDetectionService) {
            this.imageTypeIdToServiceDM.set(
                imageTypeId,
                polypDetectionServiceDM
            );
        }
        for (const imageTypeId of applicationConfig.imageTypeIdListForEsophagusDetectionService) {
            this.imageTypeIdToServiceDM.set(
                imageTypeId,
                esophagusDetectionServiceDM
            );
        }
    }

    public async detectRegionListFromImage(image: Image): Promise<Polygon[]> {
        const polypDetectionServiceDM =
            this.getPolypDetectionServiceDMForImageType(image.imageType);
        if (!polypDetectionServiceDM) {
            this.logger.error(
                "no polyp detection service corresponding to the image type, will skip"
            );
            return [];
        }
        ...
        const imageBuffer = await promises.readFile(originalImageFilePath);
        const polypDetectResponse = await this.getPolypDetectResponse(
            imageBuffer,
            polypDetectionServiceDM
        );
        const imageSharp = sharp(imageBuffer);
        return await this.getRegionListFromPolypDetectResponse(
            polypDetectResponse,
            imageSharp
        );
    }
    ...
    private getPolypDetectionServiceDMForImageType(
        ...
        const polypDetectionServiceDM = this.imageTypeIdToServiceDM.get(
            imageType.id || 0
        );
    ...
    private async getPolypDetectResponse(
        ...
        const {
            ...
            polypDetectionServiceClient.batchPolypDetect.bind(
                polypDetectionServiceClient
            ),
            { requests: [request] }
        );
        if (batchPolypDetectError !== null) {
            this.logger.error(
                "failed to call PolypDetectionService.BatchPolypDetect()",
                { error: batchPolypDetectError }
            );
        ...
        if (polypDetectResponse === undefined) {
            this.logger.error(
                "invalid response from PolypDetectionService.BatchPolypDetect()",
                { response: batchPolypDetectResponse }
            );
        }
...
export const POLYP_DETECTION_SERVICE_DETECTOR_TOKEN =
    token<PolypDetectionServiceDetector>("PolypDetectionServiceDetector");
